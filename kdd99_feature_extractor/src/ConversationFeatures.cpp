...
namespace FeatureExtractor {
	...
	void ConversationFeatures::print(bool print_extra_features,std::ostream* ostr) const
	{
		stringstream ss;
		// Intrinsic features
		...
		std::string output_str = ss.str();
    		...
    		std::ofstream* ofstream_ptr = dynamic_cast<std::ofstream*>(ostr);
   		if (ofstream_ptr && ofstream_ptr->is_open()) {
        	(*ostr) << output_str << std::endl;
        	// Thực hiện các thao tác với file ở đây
    		} else {
        	std::cout << "File is not open." << std::endl;
        	// Xử lý khi file không mở được
    		}
	}
