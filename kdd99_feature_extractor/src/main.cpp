...
void extract(Sniffer *sniffer, const Config *config, bool is_running_live)
{
	...
	while (!temination_requested && (has_more_traffic || is_running_live)) {
		...
		while ((conv = conv_reconstructor.get_next_conversation()) != nullptr) {
			...
			cf->print(config->should_print_extra_features(),&out_stream);
			delete cf;
		}
	}
...
void parse_args(int argc, char **argv, Config *config)
{
	...
	for (i = 1; i < argc && argv[i][0] == '-'; i++) {
		...
		case 'o':{
			...
			//if (argv[i] == nullptr || std::strlen(argv[i]) == 0)
			//	invalid_option_value(argv[i - 1], "", argv[0]);

			//std::streambuf* out_buffer = out_stream.rdbuf();
    			//if (out_buffer == nullptr)
       			//	 invalid_option_value(argv[i - 1], "Invalid stream buffer", argv[0]);
			//std::streambuf* out_buffer = nullptr;
			
			//out_stream.open(argv[i]);
			//if (!out_stream.is_open())
			//	invalid_option_value(argv[i - 1], "Failed to open file", argv[0]);

			//out_buffer = out_stream.rdbuf();
    			//if (out_buffer == nullptr)
        		//	invalid_option_value(argv[i - 1], "Invalid stream buffer", argv[0]);
			//std::streambuf* out_buffer = out_stream.rdbuf();
			//if (out_buffer == nullptr)
